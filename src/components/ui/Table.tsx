      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr>
            {cols.map((col) => (
              <th
                key={col.key}
                className="border-b border-slate-800 px-3 py-2 text-left text-[11px] uppercase tracking-wider text-slate-500"
              >
                {col.label}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr
              key={row.id}
              className={cn("border-b border-slate-800/60", index % 2 ? "bg-white/[0.03]" : "")}
            >
              {cols.map((col) => {
                ...
                return (
                  <td
                    ...
                    className={cn(
                      ...
                      col.mono
                        ? cn("font-semibold tabular-nums", toneMap[tone].text)
                        : "text-slate-300",
                    )}
                  >
